...
const {validateCreateUser, validateUpdateUser} = require('../validations/userValidatior') // IMPORTACION NUEVA
...
const userController = {
    ...
    async create(req, res, next){
        //validacion con Zod
        try{
            const validation = validateCreateUser(req);
    ...
    async update(req, res, next){
        try{
            ...
            if(Object.keys(validation.data).length === 0){
                throw new AppError('No se enviarion datos para actualizar', httpStatus.BAD_REQUEST);
            }
